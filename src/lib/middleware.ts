...
export function authMiddleware(request: NextRequest) {
  try {
    ...
    if (jalanPublik.includes(pathname)) {
      if (token) {
        ...
        if (dataPengguna) {
          ...
          if (pengguna.peran.includes('Super Admin') || pengguna.peran.includes('Admin')) {
    ...
    if (isJalanTerproteksi) {
      ...
      try {
        const pengguna = JSON.parse(dataPengguna)
        const peranDibutuhkan = Object.entries(jalanTerproteksi).find(([path]) => 
          pathname.startsWith(path)
        )?.[1]

        // Jika pengguna tidak memiliki peran yang dibutuhkan, redirect ke home
        if (peranDibutuhkan && !peranDibutuhkan.some(peran => pengguna.peran.includes(peran))) {
