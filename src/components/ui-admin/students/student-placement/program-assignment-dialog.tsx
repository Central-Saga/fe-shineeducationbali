...
const formSchema = z.object({
  packageId: z.string({
    required_error: "Pilih paket program",
  }),
});
...
export function ProgramAssignmentDialog({
  ...
  const handleAssignment = async () => {
    try {
      setIsLoading(true);
      // Di sini implementasi logika untuk mendaftarkan siswa ke paket program
      // Contoh mock:
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      toast.success(`${selectedStudents.length} siswa berhasil didaftarkan ke paket program`);
      onSuccess?.();
      onOpenChange(false);
    } catch (error) {
      console.error("Failed to assign programs:", error);
      toast.error("Gagal mendaftarkan siswa ke paket program");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={isLoading ? undefined : onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Pendaftaran Paket Program</DialogTitle>
          <DialogDescription>
            Pilih paket program untuk {selectedStudents.length} siswa terpilih
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          ...
          >
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Paket Program</FormLabel>
                  <Select
